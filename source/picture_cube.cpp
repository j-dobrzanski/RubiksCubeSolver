...
static inline int check_solution_side(std::vector<std::vector<PictureCubeTile*>*>* current_side, uint8_t cube_size);

PictureCube::PictureCube(uint8_t size, CubeType type){
...
int PictureCube::rotate(Axis axis, uint8_t tile_index){
    switch(axis){
        case Axis::X:
            if(tile_index == 0){
