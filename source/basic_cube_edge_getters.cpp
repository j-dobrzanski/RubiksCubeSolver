...
static std::array<std::pair<Side, Side>, EDGE_LENGTH> reverse_edge(std::array<std::pair<Side, Side>, EDGE_LENGTH> edge);

std::pair<Side, Side> BasicCube::get_solved_edge(Side side1, Side side2){
...
std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_single_edge(Side side1, Side side2){
    switch(side1){
        case Side::Left:
            switch(side2){
                case Side::Left:
                    return std::array<std::pair<Side, Side>, EDGE_LENGTH>();
                    break;
                case Side::Right:
                    return std::array<std::pair<Side, Side>, EDGE_LENGTH>();
                    break;
...
std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_LU(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_left)[0][CUBE_SIZE_LAST_INDEX - index - 1].side_destination, (*this->side_up)[CUBE_SIZE_LAST_INDEX - index - 1][0].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_LD(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_left)[CUBE_SIZE_LAST_INDEX][CUBE_SIZE_LAST_INDEX - index - 1].side_destination, (*this->side_down)[CUBE_SIZE_LAST_INDEX - index - 1][0].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_RD(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_right)[CUBE_SIZE_LAST_INDEX][index + 1].side_destination, (*this->side_down)[CUBE_SIZE_LAST_INDEX - index - 1][CUBE_SIZE_LAST_INDEX].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_RU(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_right)[0][index + 1].side_destination, (*this->side_up)[CUBE_SIZE_LAST_INDEX - index - 1][CUBE_SIZE_LAST_INDEX].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_LB(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
...
std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_LF(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_left)[index + 1][CUBE_SIZE_LAST_INDEX].side_destination, (*this->side_front)[index + 1][0].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_RF(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_right)[index + 1][0].side_destination, (*this->side_front)[index + 1][CUBE_SIZE_LAST_INDEX].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_RB(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_right)[index + 1][CUBE_SIZE_LAST_INDEX].side_destination, (*this->side_back)[index + 1][CUBE_SIZE_LAST_INDEX].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_FU(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_front)[0][index + 1].side_destination, (*this->side_up)[CUBE_SIZE_LAST_INDEX][index + 1].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_FD(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_front)[CUBE_SIZE_LAST_INDEX][index + 1].side_destination, (*this->side_down)[CUBE_SIZE_LAST_INDEX][index + 1].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_BD(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
        edge[index] = std::make_pair((*this->side_back)[CUBE_SIZE_LAST_INDEX][index + 1].side_destination, (*this->side_down)[0][index + 1].side_destination);
    }
    return edge;
}

std::array<std::pair<Side, Side>, EDGE_LENGTH> BasicCube::get_edge_BU(){
    auto edge = std::array<std::pair<Side, Side>, EDGE_LENGTH>();
    for(size_t index = 0; index < EDGE_LENGTH; index++){
